...
- `internal/notify` fans a message out to the user's enabled channels
  ...
  TUI for its whole run); interactive agents use `SendAway`, while routine
  agents always push, so autonomous alerts arrive even when a UI is connected.
  Messages may include a channel-native URL.
...
- `deploy/provision.sh [name]` creates a Hetzner VM (tokens from env or
  ...
  service, `tailscale serve` over HTTP (`orc-cert.service` adds the HTTPS
  handler once the ACME cert lands), nightly self-update timer,
